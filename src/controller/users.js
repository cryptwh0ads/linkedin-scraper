...
const login = ({ db, jwt, jwtSecret }) => async (req, res) => {
  ...
  if (users.length === 1) {
    if (bcrypt.compareSync(req.body.passwd, users[0].passwd)) {
      const { id, name, email, role, socialEmail, socialPasswd } = users[0];
      const user = {
        id,
        name,
        email,
...
const create = ({ db }) => async (req, res) => {
  ...
  let socialHash = bcrypt.hashSync(newUser.socialPasswd, 10);
  const userToInsert = {
    name: newUser.name,
    email: newUser.email,
    passwd: hash,
    socialEmail: newUser.socialEmail,
    socialPasswd: socialHash,
  };
...
const update = ({ db }) => async (req, res) => {
  ...
  fields.forEach((field) => {
    if (updatedUser[field]) {
      ...
      } else if (updatedUser.socialPasswd) {
        let socialHash = bcrypt.hashSync(updatedUser.socialPasswd, 10);

        userToUpdate[field] = socialHash;
      } else {
  ...
  await db("users").where("id", id).update(userToUpdate);
